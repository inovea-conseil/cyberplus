 ...
 */
class pdf_crabe_cyberplus extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "crabe_cyberplus";
		$this->description = $langs->trans('PDFCrabeCyberPlusDescription');
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		$outputlangs->load("cyberplus@cyberplus");
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
            ...
            $cbId = $conf->global->PAYMENT_ID ? $conf->global->PAYMENT_ID : 0;
            
            $generate_link = true;	    
            if ($cbId > 0)
            {
                if ($object->mode_reglement_id != $cbId)
                {
                    $generate_link = false;	               
                }                
            } 
            ...
            if ($generate_link)
            {
				...
				$payment_link = $cyberplus->getLink($object);
